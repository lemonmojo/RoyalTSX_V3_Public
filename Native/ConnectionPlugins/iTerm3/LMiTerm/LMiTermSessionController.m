...
@implementation LMiTermSessionController {
...
- (instancetype)initWithProfile:(Profile*)profile command:(NSString*)command initialSize:(NSSize)initialSize
{
    ...
    if (self) {
        ...
        if (!aDict) {
            aDict = [LMiTermSessionController defaultProfile];
        }
        ...
        if (session.view) {
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   selector:@selector(sessionViewDidMoveToWindow:)
                                                       name:LMSessionViewDidMoveToWindowNotification
                                                     object:session.view];
        }
...
- (void)windowDidBecomeKey:(NSNotification*)aNotification
{
    ...
    for (LMiTermSessionController* session in self.delegate.sessions) {
...
- (void)safelySetSessionSize:(PTYSession*)aSession rows:(int)rows columns:(int)columns
{
    ...
    // LMiTerm: We don't need this (hopefully)
...
- (PTYSession *)createTabWithProfile:(Profile *)profile
    ...
    // If a command was provided, create a temporary copy of the profile dictionary that runs the user-supplied command in lieu of the profile's command.
    ...
    [self setupSession:aSession title:@"Session" withSize:nil];
...
- (void)setActiveSessionOnMainThread:(PTYSession*)session
{
    ...
    LMiTermSessionController* sessionController = [self.delegate sessionControllerForPTYSession:session];
...
- (PTYWindow *)ptyWindow { return nil; }
...
- (int)windowType { return WINDOW_TYPE_NORMAL; }
